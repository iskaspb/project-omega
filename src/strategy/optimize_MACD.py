...
def main() -> None:
    ...
    for symbolIndex in range(0, len(symbols)-1):
        ...
        print(f"#optimize for symbol {symbol} using command : {' '.join(optimizeCmd)}", flush=True)
        ...
        if x:
            print(f"#Optimization of {symbol} failed, please review optimization criterias - you can repeate the optimization command above to get more insights")
            #print(output)
            continue
        ...
        if x:
            ...
            print(f"ERROR: Couldn't interpret the output symbol {symbol}")
            print(output)
            continue
        ...
        print(f'self.MACDIndicator("{symbol}", {fast}, {slow}, {signal}, interval) #Total Trades: {totalTrades}, Compounding Annual Return: {annualInterest}, Sharpe Ratio: {sharpeRatio}', flush=True)
